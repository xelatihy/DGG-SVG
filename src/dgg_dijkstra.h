...
private:
// fabio: added to make it compile
  using SparseGraph<T>::graph_neighbor;
  using SparseGraph<T>::graph_neighbor_dis;
  using SparseGraph<T>::node_number_;
  using SparseGraph<T>::num_edges_;
  using SparseGraph<T>::average_degree_;
// fabio: added to make it compile
...
public:
  ...
  inline T distanceToSource(int index) override {
